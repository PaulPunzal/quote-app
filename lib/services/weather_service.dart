...
class WeatherSnapshot {
  ...
  final double? tempC;
  final String locationLabel; // city name only, e.g. 'Marilao'
...
class WeatherService {
  ...
  ///
  /// Each result's full [GeocodeResult.displayLabel] (city, region,
  /// country) is what the picker list shows, so you can tell apart two
  /// same-named cities in different countries — but only the bare city
  /// [GeocodeResult.name] gets persisted once you pick one (see
  /// [saveLocation]).
  ...
  Future<void> saveLocation(GeocodeResult result) async {
    ...
    // Only the city name is kept as the label -- once you've picked the
    // right match from the disambiguated search list, the region/
    // country don't add anything to a short on-screen label like
    // "Hot in Marilao", they just make it longer.
    await prefs.setString(_keyLabel, result.name);
    await prefs.setDouble(_keyLat, result.latitude);
    await prefs.setDouble(_keyLon, result.longitude);
    // The cached reading described the OLD location -- keeping it
    // around would silently misattribute weather to a place that's no
    // longer configured. Clear it so the next fetch starts fresh (and
    // offline-before-first-fetch just means no weather context yet,
    // same as it is today).
